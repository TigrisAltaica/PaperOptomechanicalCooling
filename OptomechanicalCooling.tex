...
\begin{equation}
\dot{\rho} = \mathcal{L} \rho = \frac{1}{i\hbar}[H,\rho]+L_a\rho, 
\end{equation} with
...
\end{equation} These eigenvalues correspond only to the $L_a$ term. The full eigenvalues for $\mathcal{L}$ would also include the eigenvalues for the Hamiltonian term. In this section we focus only on the Lindblad term for clarity, but this is an important distinction.
...
\eqref{eq:projected_master_equation} we write equation \eqref{LCMasterEq} as
\begin{equation}
...
\end{equation}
gives the field-mechanic oscillator interaction. We employ projection
...
\item $ P\mathcal{L}_{0} = \mathcal{L}_{0}P = 0 $\qquad as the
  corresponding eigenvalues are 0 \fxnote{No debe de ser la parte
    real?}
...
\end{enumerate} In this case $P=P_m^{\lambda_m^0}P_c^{\lambda_c^0}$
\fxnote{Los operadores $P_m$ y $P_c$ no estan definidos en ningun
  lugar, me parece que los debes de introducir despues}. We project
...
\check{\rho}_{\lambda'}$ corresponding to the damping basis states
seen in Appendix \ref{App1}, plus the states for the $H$ terms
\fxnote{Aqui hay algo mal, los operadores que defines solo actuan en
  el espacio del campo y no en el espacio completo del sistema. De
  hecho aqui deberias de introducir los operadores P como producto de
  proyectores en el espacio del campo de la cavidad producto el
  espacio del oscilador.}. $\lambda, \lambda'$ represent both the
damping basis eigenvalues plus the eigenvalues for the $H$ terms,
separated by time scale \fxnote{Es mas claro si pones directamente a
  que es igual $\lambda$ y $\lambda'$}. \fxnote{Creo que aqui debes
  definir explicitamente quien es P y Q}. We may thus evaluate the
$\mathcal{L}_0$ terms directly by applying them to the nearest states
