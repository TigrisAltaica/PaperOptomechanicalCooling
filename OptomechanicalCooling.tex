...
\eqref{eq:projected_master_equation} we first separate  $\mathcal{L}$ into
\begin{align}
...
\end{equation} so that $\mathcal{L}=\mathcal{L}_0+\mathcal{L}_1$. We also employ projection operators like those in \cite{CarmichaelQO}. These projection operators fulfill a completeness relation
...
\item $ P\mathcal{L}_{0} = \mathcal{L}_{0}P = 0 $\qquad as the corresponding eigenvalues are 0
...
\end{enumerate} In this case $P=P_m^{\lambda_m^0}P_c^{\lambda_c^0}$. We project the master equation $\dot{\rho}=\mathcal{L}\rho$ into both $P$ and $Q$ spaces and insert the completeness relation to obtain two equations. Working in the decay picture
...
\end{equation} We decompose the projectors into the forms $Q=\sum_\lambda \hat{\rho}_\lambda \otimes \check{\rho}_\lambda$ and $P=\sum_{\lambda'} \hat{\rho}_{\lambda'} \otimes \check{\rho}_{\lambda'}$ corresponding to the damping basis states seen in Appendix \ref{App1}, plus the states for the $H$ terms. $\lambda, \lambda'$ represent both the damping basis eigenvalues plus the eigenvalues for the $H$ terms, separated by time scale. We may thus evaluate the $\mathcal{L}_0$ terms directly by applying them to the nearest states
