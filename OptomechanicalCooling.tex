...
\end{equation} this term arises due to the explicit time dependence in the Floquet operators as they do not, in general, commute with their own time derivatives.
...
Everything is calculated using $\frac{\delta}{\nu_0}$ as the variable.
...
\begin{figure}[h!]
\includegraphics[scale=.5]{TempGorden1+.pdf}   
\caption{ Mechanical oscillator phonon number with and without a time dependence. The negative temperature is interpreted as heating of the resonator.}\label{fig:TempComparisson+}
\end{figure}


For figures \ref{fig:TempComparisson-} and \ref{fig:TempComparisson+},we used $\kappa = 0.05\nu_0$ and
calculated $\expval{m}$ near the points $\frac{\delta}{\nu_0}=-1$ and $\frac{\delta}{\nu_0}=1$ respectively. The
positive ratio corresponds to heating of the resonator
...
as $|\frac{\delta_t}{\nu_0}|,\frac{\delta_t}{\delta^2}\ll 1$.  The temperature behavior is the same around both minimums. In both cases the correction pushes the temperature closer to the limit case where the non-dominant sideband is zero, as can be seen from equation \eqref{eq:TempExpression}. 
...
In Appendix \ref{CoolingAppendix} we consider a harmonic oscillator with no damping, so the left and right eigenstates of the damping basis reduce to 
...
\begin{equation}
\lambda_c = ij,
\end{equation} with $i$ the imaginary unit and $j$ the cavity quantum number. If we then set $t_0=0$ and note that
...
\end{equation}
 ...
 and equation \eqref{CoolingBeforecoefficients} can then be straightforwardly arranged in the form \eqref{eq:ProyectedMasterEqCooling}. When arranging in this form, the coefficients that are obtained are complex. The real part corresponds to the cooling and heating rates and the imaginary part corresponds to a negligible correction to the mechanical frequency $\nu_0$.
