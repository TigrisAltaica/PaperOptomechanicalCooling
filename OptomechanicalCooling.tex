...
\end{equation}\fxnote{Por aqui hay que poner quien es $C(t)$}
...
\fxnote*{Usando la ecuacion para C(t), explica mejor porque lo desprecias}{Finally, as the $C(t)$ term is proportional to
$\epsilon\abs{\beta_1}^2$ given the approximation in equation
\eqref{eq:DotGammavsGamma}, so it is neglected,} and the master
...
\fxnote{Pon quien es $\overline{H}$ aqui, y tambien en el apendice, ya
  que no aparece en ningun lado}$\overline{H}$ is a term proportional
...
\end{equation}
\fxnote{En ningun lugar esta escrito explicitamente quienes son $A_-$
  y $A_+$, lo puedes poner?}.
...
\ref{CoolingAppendix}\fxnote{Tienes que poner explicitamente como
  hiciste el promedio, ya sea aqui o, de preferencia, en el apendice}.
With these expressions in hand, the temperature is easily calculated.
\fxnote*{Creo que solo las desintonias, pon explicitamente que es lo
  que normalizas}{Everything} is first normalized with respect to $\nu_0$ and the graph
...
\fxnote*{A esta frase le falta algo}{It must be noted that since terms
  of order $\frac{\delta_t}{\delta^2}$ the approximation does not hold
  as $\delta$ approaches 0.} The approximation is valid around
$\frac{\delta}{\nu_0}\approx -1$, since we only keep one term for the
correction to $\delta$. \fxnote{Aqui estaria muy bueno que puedas
  poner explicitamente para que parametros falla, es decir, porque
  para $\delta=-1$ se satisfacen todas las condiciones y a partir de
  que valor falla}
\fxnote{Creo que tienes razon, la figura 3 no parece aportar nada. La
  figura 2 me parece que esta buena, tienes que hacer referencia a
  ella en el texto. Nos falta una explicacion de porque esto funciona:
Checaste si hay mucha diferencia con los A que calientan? Capas que
los A que enfrian son casi iguales, pero los otros son bastante diferentes.}
