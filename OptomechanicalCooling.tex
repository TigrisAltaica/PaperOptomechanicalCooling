...
\end{equation} and its complex conjugate.  For simplicity we employ the normalization $\omega=1$ so that, numerically $\nu_0=n$. 
...
\end{align}
\fxnote*{No es claro como lo haces, lo puedes poner de forma mas
  explicita?}{This means that an additional term should be considered in the $\beta$
  equation \eqref{eq:displaced_frame}, $-i\nu_0$, as the term involving $\dot{\Gamma}$ and $\dot{\Gamma}^\dagger$ is completely analogous to the laser pump term after perfoming this substitution and can also be eliminated by the reference frame shift. This means $\alpha$ and $\beta$ must actually fulfill the differential equations}

\begin{align}\label{eq:displaced_frame_dot_gamma}
\dot{\alpha} =& \alpha(-\frac{\kappa}{2}+i(\delta+g\sqrt{\frac{\hbar}{2M}}(\gamma_-(t) \beta^* + \gamma_+(t) \beta))-i\frac{\Omega}{2},\\
...
\eqref{SmallOscillationsTDHO} in hand, we can solve
Eq.~\eqref{eq:displaced_frame}. Assuming that the time scales of the
dissipative processes are fast compared with the other time scales we
can focus on the stationary case ($\dot{\alpha}(t)=\dot{\beta}(t)=0$);
we also assume a weak coupling regime (coefficients of second order in
$g$ or higher are neglected) obtaining
...
\end{equation}
and  $\frac{W}{|f|^2} \approx \nu_0$. These expressions neglect terms of order $\frac{\epsilon}{n^2}$  since we work in the regime where $n \gg 1$. Using \eqref{eq:alpha_beta_sta} in~(\ref{eq:hamiltonian_no_small})
...
Our focus is on the parameter regime where the coupling is weak enough that \fxnote*{Poner porque}{}
$\frac{\delta_t}{\delta}\ll 1$.
...
reality cooling towards a \fxnote*{poner cual coherent state es al que
  evoluciona en el laboratory frame}{coherent state in the laboratory
  frame, $\ket{\beta_1}$}. It's important to note that these excitations correspond to
a harmonic oscillator with a time-dependent frequency and can't
necessarily be directly compared with the excitations of a harmonic
