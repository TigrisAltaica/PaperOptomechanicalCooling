...
\fxnote{Creo que tiene más sentido dejarlo donde está, al fin y al cabo la temperatura está medida en el marco desplazado y esto todavía no está en ese marco}
...
=& \delta + \delta_t\cos(\nu_0 t),
\end{align} with $\delta_t=\frac{\chi_0^2 \abs{\alpha_1}^2}{\gamma}$.  We work in a weak coupling regime, so we require the coupling $\chi_0$ to be small enough so that $\delta_t\ll\delta$. As $\Omega$ is large, we may work with the second order of the coupling parameter in this expression. This correction does not go into the expression for $\alpha_1$ because when doing a Taylor expansion the first correction term would be of the order $\frac{\delta_t}{\delta^2}$ which we may safely neglect.
...
\end{equation} with $|\gamma|^2 = \gamma_+(t) \gamma_-(t)$ which is not time dependent at our level of approximation, plus a correction for each one, proportional to  $C_t$. These corrections are
...
non-driven case over a range around the minimum point.  \fxnote*{Dada la relación de scattering es cierto pero de manera un poco trivial, ¿no? Como $\nu_0=n\omega$ la frecuencia secular no es libre y siempre se puede eliminar. Quité la frase e hice la lista de cosas asumidas. También re-escribí un poco la sección donde se pone la solución de $\alpha$ y $\beta$ para que fuera coherente con lo que pedimos.}{It is worth remembering the assumptions made to this point. In order to derive the cooling rates we have neglected terms of order $\epsilon^2$, $\frac{\epsilon}{n^3/2}$ and smaller and terms of order $\frac{\delta_t}{\delta^2}$ and smaller.}
...
\lambda_c = ij,
\end{equation}
with $i$ the imaginary unit and $j$ the cavity quantum number. If we
then set $t_0=0$ and note that $\mathcal{L}_0^t(0)=0$, we may write
\eqref{ProyectionEQ}, returning $P$ and $Q$ to their original
notation, as
...
\end{align} the Kronecker deltas refer to the numbers in the eigenvalues, both for the cavity and the mechanical oscillator. These eliminate the sum over all possible $\lambda'$ values. This eigenvalue selection then determines the value of the functions $E(\lambda')$ and $E_c(\lambda')$ which are then averaged over time using

\begin{equation}
<E(\lambda)> = \frac{2\pi}{\nu_0}\int_0^\frac{2\pi}{\nu_0}dt E(\lambda) 
\end{equation}
and equation \eqref{CoolingBeforecoefficients} can then be
straightforwardly arranged in the form
...
\begin{equation}
A_+ = - \frac{1}{2}\sum_{\lambda'}\hbar^2\chi_0^2 |\alpha_1|^2 |\gamma^2|(\delta_{l,1}\delta_{j,-1}+\delta_{l,-1}\delta_{j,1})\delta_{n,0}\expval{E(\lambda'}.
\end{equation} This process neglects the mechanical damping as this process occurs on the longest timescale of all \cite{BarberisLC}. The $L_\Gamma$ term can simply be added to equation \eqref{eq:LaserCoolingEqAppendixB} after the cooling procedure is complete.
\fxnote{Creo que así queda bastante claro}
