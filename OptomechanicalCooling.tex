...
P e^{-L_0 t}L_1e^{L_0 t}\int_{t-\Delta t}^{t}dt'Qe^{-L_0 t'}L_1e^{L_0 t'}Pe^{-L_0( t-\Delta t)}\rho(t-\Delta t).
...
\begin{align}
P e^{-L_0 t}L_1[&\sum_{\lambda',\lambda}\int_{t-\Delta t}^{t}dt'e^{\lambda' t} \hat{\rho}_{\lambda'} \otimes \check{\rho}_{\lambda'}e^{-\lambda' t'}L_1\\
&e^{\lambda t'}\hat{\rho}_\lambda \otimes \check{\rho}e^{-\lambda( t-\Delta t)}\rho(t-\Delta t)]\nonumber.
\end{align} We wish to take the limit when $\Delta t \to \infty$. In this limit

\begin{equation}
e^{\lambda' \Delta t} \to 1, \qquad \qquad e^{\lambda \Delta t} \to 0.
\end{equation} This is because $\lambda'$ represents the slow time scale and $\lambda$ represents the fast time scale. Only the stationary sttes survive as all other eigenvalues have a negative real part.  We multiply both sides by $Pe^{L_0 t}$ remembering that $P^2 = P$ We then trace over the cavity degrees of freedom. As has already been noted, the Floquet operators can leave the integral and also leave the trace. We assume that the initial condition can be expressed as a separable state $\rho = \rho_m \otimes \rho_st$ and only $\rho_st$ must remain within the trace. Using $\mu=Tr_a[P\rho]$ we arrive at the equation

\begin{equation}
\dot{mu} = (L_1)_{\Gamma}(L_1)_{\Gamma}\mu \int_0^{\infty} e^{i\nu(t)t} Tr_a[(L_1)_a e^{L_at}(L_1)_a\rho_{st}],
\end{equation} which then leads directly to \eqref{eq:projected_master_equation}.
