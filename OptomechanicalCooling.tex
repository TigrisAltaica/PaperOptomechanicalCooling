...
\eqref{eq:projected_master_equation} we begin with the equation 
...
projects the system in a fast-decaying evolution space, they fulfill
...
\item $ P\mathcal{L}_{0} = \mathcal{L}_{0}P = i w $\qquad with $w$ a
  real number, because P projects the state in its stationary
  part\fxnote{Checar}
...
We project the master equation \eqref{eq:master_decay_picture} into
both $P$ and $Q$ to obtain the equations (we ommit the primes from now on)
\begin{align*}
...
\end{align*}
and this is substituted into the $P$ equation \fxnote{comentar algo de la aproximacion}
...
\end{align}
where only the first integral term is non zero \fxnote{Creo que esto
  solo es verdad si la condicion inicial no incluye terminos en Q. Es
  asi?}. We focus on this term and transform back from the decay
picture \fxnote{Creo que como regresas a la ``picture'' estandar, es confuso que hayas eliminado las primas}

\begin{equation}\label{eq:temp_1}
P\dot{\rho}(t)=P e^{-\mathcal{L}_0 t}\mathcal{L}_1e^{\mathcal{L}_0 t}\int_{t_0}^{t}dt'Qe^{-\mathcal{L}_0 t'}\mathcal{L}_1e^{\mathcal{L}_0 t'}Pe^{-\mathcal{L}_0(t_0)}\rho(t_0).
\end{equation}\fxnote{Le agregue el lado izquierdo a la ecuacion de arriba, es asi? o tiene exponenciales por regresar a la picture normal?}
...
\end{eqnarray}
the operators with the $\lambda'$ label project the state in the
slow-decaying time-scale subspace, they are eigenstates of
$\mathcal{L}_0$ with imaginary eigenvalue. The operators with the
...
part, those states decay quickly. Using the states defined in Appendix
\ref{App1},
$\hat{\rho}_{\lambda'}^{cav}=\hat{\rho}_n^l,\,\check{\rho}_{\lambda'}^{cav}=\check{\rho}_n^l$
with $n=l=0$, and
$\hat{\rho}_{\lambda}^{cav}=\hat{\rho}_n^l,\,\check{\rho}_{\lambda}^{cav}=\check{\rho}_n^l$
with $n+|l|>0$. The mechanical states are the left and right
eigenstates of $\mathcal{L}_{mec}$
\begin{equation}
\hat{\rho_{\lambda_m}^{mec}}=\Ket{n}\Bra{n+m}=\check{\rho_{\lambda_m}^{mec}}^\dagger,
\end{equation} \fxnote{checa que puse bien los estados}
with eigenvalues
\begin{equation}
\lambda_m = -i m \nu(t).
\end{equation}
...
\end{align}
\fxnote*{aqui esto no es necesario, o si?}{as the cavity is taken to
  be at zero temperature}
\fxnote*{esto no va a aqui}{Here we make an important note. In
...
\end{equation}
where we used that
$\lambda' = \lambda_{mec}(t)=i\nu_0 + i\epsilon' \cos(2\omega t)$ and
$\epsilon\ll \nu_0$, each term is now integrated separately. For the
first term
...
\end{equation} \fxnote{Aqui me perdi, si sustituiste $\lambda'$ en una ecuacion ya no deberia aparecer en la segunda}


The adiabatic approximation consists of taking the limit
$(t-t_0) \to \infty$ or $t \to \infty, \quad t_0 \to 0$. In this limit
