...
Hamiltonian, that includes terms that appeared when making the
transformation on the $L$ operators, is given by \fxnote{Valdra la pena decir que terminos?}
\begin{align*}
...
\end{align}
\fxnote{Comentamos de que parte de los terminos extras del Hamiltoniano aparecen cuando}
...
\fxnote*{Esto es debido a los operadores L o a H?}{The transformed Hamiltonian also contains a pump-like term that
depends on the time derivative of the Floquet operators}
...
maintains the previous ratio between the two frequencies. \textbf{This chooses a certain solution for the MAthieu equation. What does it mean physically?} As a
...
\textbf{Here should be a discussion of the results: What is  the reason for the shift, how does the temperature change when one changes the paramerters, comparison with the result fro nour old paper, etc.}



\begin{figure}
\includegraphics[scale=.30]{GraficaTemp.pdf} 
\caption{\textit{Comparison between the results for temperature with
    and without the correction term} The dotted line represents the
  standard non-time dependent prediction and the solid line represents
  our prediction with the correction term. The correction results in a
  shift of the location of the minimum temperature, as well as a lower
  minimum. Besides all the scattering relations already stated, we
  work under $\kappa \ll \nu_0$}
\label{GraficaTemp}
\end{figure}
