...
\affiliation{Instituto de Investigaciones en Matemáticas Aplicadas y en Sistemas,
Universidad Nacional Autónoma de México, Circuito Escolar s/n Ciudad Universitaria México, D.F.}%
...
The temperature shift depends on $\epsilon$. As long as $\nu_0$ and $\omega$ fulfill \eqref{scattering}, altering them does not seem to alter the behavior of the final temperature. This indicates that the shift is related to the time dependent nature of the correction. One possible explanation for this shift is that the $\epsilon$ term causes the cooling and heating coefficients to shift away from each other.  After the shift, the resonance point with each of the coefficients is further away from the optimal point for the other coefficient. The distribution for both coefficients becomes narrower and presents a higher peak. This effect can be observed in figure \ref{GraficaA-}. The parameters used appear reasonable, as controlled frequency modulation has been shown \cite{WoolleyNM}\cite{JockelS}.

\begin{figure}
\includegraphics[scale=.4]{GraficaA-.pdf}
\caption{Cooling coefficient $A_-$ for different perturbation levels. As $\epsilon$ increases the $A_-$ presents a narrower distribution and a higher peak. The peak shifts away from the cavity resonance corresponding to $\nu_0$. $A_+$ is not shown as, in this region, it is around 4 orders of magnitude smaller than $A_-$.  Besides all the scattering relations already stated, we
  work under $\kappa \ll \nu_0$}
\label{GraficaA-}
\end{figure}
