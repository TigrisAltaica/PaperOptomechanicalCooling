...
$\omega$ fulfill \eqref{scattering}, altering them does not seem to
alter the behavior of the final temperature. This indicates that the
shift is related to the time dependent nature of the correction. One
possible explanation for this shift is that the $\epsilon$ term causes
the cooling and heating sidebands to shift away from each other. The
common crossing point indicates the point where the resonance is
placed at a symmetry point for the sideband's position and thus the
effect cancels out. After the shift, the resonance point with each of
the sidebands is further away from the optimal point for the other
sideband. This effect can be observed in figure \ref{GraficaA-}. The
parameters used appear reasonable, as controlled frequency modulation
has been shown \cite{WoolleyNM}\cite{JockelS}.
