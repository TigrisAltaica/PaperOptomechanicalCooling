...
The derivation in Appendix \ref{CoolingAppendix} includes a time integration. However, since the explicit time dependence in the Floquet Operators takes the form of \eqref{SmallOscillationsSolution} (and it's time derivative and their complex conjugates), and since $\nu_0 = n\omega$, this explicit dependence is canceled out by the coeffcients \eqref{GammaCoefficients}, save for terms that are of the order $\frac{\epsilon}{n\nu_0}$. Thus, we label these operators as $\Gamma_0, \Gamma^\dagger_0$ and treat them as non time dependent operators.  \fxnote{Esto no se entiende
  bien}
...
\end{equation} These are the right eigenstates, which are the eigenvalues of the equation $L\hat{\rho}_n^l = \lambda_n^l\hat{\rho}_n^l$, however the left eigenstates must also be considered, these are the eigenvalues of the equation $\check{\rho}_n^lL = \lambda_n^l\check{\rho}_n^l$ and have the same eigenvalues. These are
